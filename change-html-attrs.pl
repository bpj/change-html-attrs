...
L<< Pod::Simple::XHTML|Pod::Simple::XHTML >> and L<< perldoc|perldoc >>,
and then converting the output of that to ODT with
L<< pandoc|http://pandoc.org >>. Piping from perldoc to pandoc loses
some formatting for some reason!
