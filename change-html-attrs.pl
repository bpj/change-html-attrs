...
as output by e.g. by the XHTML export filter of LibreOffice, which uses
C<< <span> >> elements with classes and a corresponding embedded CSS
stylesheet with styles like C<< .T2 { font-style:italic; } >> rather
than C<< <em> >> elements. I have been told (I'm on Linux) that Apple's
TextUtil, at least when converting from RTF to HTML even replaces
headers with styled C<< <p> >> elements! Clearly these tools reflect the
way the data are represented in the original format, whether ODF XML or
RTF too shallowly.

Unfortunately the correspondence between class names and styles
generated automatically by LibreOffice or an RTF writer is not constant,
so one time you may get C<< .T2 { font-style:italic; } >> and another
time you get C<< .T5 { font-style:italic; } >> or some other random,
auto-generated class name for what should be an C<< <em> >> element. To
work around this you can use CSS property names and values as search
criteria, and if the HTML file contains any C<< <style> >> elements
these will be analysed to identify classes, ids and/or tags
...
The YAML config file to use. See L<< /The YAML config file >> for its
...
in the output of LibreOffice's XHTML exporter. Note that strings
